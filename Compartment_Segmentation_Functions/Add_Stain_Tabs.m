...
function Add_Stain_Tabs(app,Current_Seg_Params)

stain_names = app.Stain_Names;
delete(get(app.StainsTabGroup,'Children'))

% butt_idx order = [colorspace, color deconvolution, custom path]
...
if ismember(butt_idx,[1,2])
    ...
    for i = 1:length(stain_names)
        ...
        stain_tab = uitab(app.StainsTabGroup,'Title',stain_names{i},'Scrollable','on','AutoResizeChildren','on');
        ...
        if strcmp(stain_names{i},'Hematoxylin')
            ...
            slider.Position = [467 19 3 107];
            slider.Value = 5;
    
        end
        
        % Adding sub-compartment segmentation parameters from previous run
        % through
        if nargin==2
            stain_field = strcat('Stain',num2str(i));

            channel_drop.Value = Current_Seg_Params.CompartmentSegmentation.(stain_field).Channel;
            threshold_field.Value = Current_Seg_Params.CompartmentSegmentation.(stain_field).Threshold;
            if Current_Seg_Params.CompartmentSegmentation.(stain_field).ThresholdDir
                threshold_switch.Value = 'above';
            else
                threshold_switch.Value = 'below';
            end
            minsize_field.Value = Current_Seg_Params.CompartmentSegmentation.(stain_field).MinSize;
            slider.Value = Current_Seg_Params.CompartmentSegmentation.(stain_field).Splitting;
        end
