...
if ismember('Colorspace',seg_fields)
    method_idx = 1;
elseif ismember('ColorDeconvolution',seg_fields)
    method_idx = 2;
elseif ismember('Path',seg_fields)
    method_idx = 3;
end

if ismember(method_idx,[1,2])
    stains = seg_fields(contains(seg_fields,'Stain'));
    
    if method_idx==1
        % Using color transforms
        colorspace_idx = Seg_Params.Colorspace;
    ...
    elseif method_idx==2
        ...
        stain_abbrevs = {'H&E','H&E 2','H DAB','H&E DAB','H AEC',...
        'FastRed FastBlue DAB','Methyl Green DAB','Azan-Mallory','Alcian blue & H',...
        'H PAS','RGB','CMY'};
        [stain1,stain2,stain3] = colour_deconvolution(img,stain_abbrevs{Seg_Params.ColorDeconvolution});
    ...
    % Temporary fix for non triple combos
    if size(comp_img,3)<3
        comp_img = cat(3,comp_img,zeros(size(mask)));
    end

else
    % This is for custom path selection
