...
if isempty(app.Notes_File)
    ...
    notes_dir = strcat(pwd,filesep,'HistoLens_Notes_Archive',filesep);
    ...
    notes_filename = strcat(notes_dir,slide_dir,'_',current_time,'.csv');

    app.Notes_File = notes_filename;
    
    % Getting notes format
    all_img_names = app.Full_Feature_set.ImgLabel;
    blank_notes = cell(length(all_img_names),1);

    notes_table = cell2table([all_img_names,blank_notes],'VariableNames',...
        {'ImgLabel','Notes'});
    
    app.Notes = notes_table;
    writetable(app.Notes, app.Notes_File);
...
if ~app.Comparing
    ...
    if ~isempty(app.Notes_edit.Value)
        selected = app.Image_Name_Label.Value;

        selected = strsplit(selected,',');
        selected = selected{1};

        app.Notes.Notes{strcmp(app.Notes.ImgLabel,selected)} = app.Notes_edit.Value{1};
    ...
    if ~isempty(app.Img_one_Edit.Value)
        ...
        app.Notes.Notes{strcmp(app.Notes.ImgLabel,selected1)} = app.Img_one_Edit.Value{1};
    ...
    if ~isempty(app.Img_two_Edit.Value)
        ...
        app.Notes.Notes{strcmp(app.Notes.ImgLabel,selected2)} = app.Img_two_Edit.Value{1};
