...
if isempty(app.Notes_File)
    ...
    notes_filename = strcat(notes_dir,slide_dir,'_',current_time,'_',app.Structure_Idx_Name,'.csv');

    app.Notes_File.(app.Structure_Idx_Name) = notes_filename;
    
    % Getting notes format
    all_img_names = app.Full_Feature_set.(app.Structure_Idx_Name).ImgLabel;
    blank_notes = cell(length(all_img_names),1);

    notes_table = cell2table([all_img_names,blank_notes],'VariableNames',...
        {'ImgLabel','Notes'});
    
    app.Notes.(app.Structure_Idx_Name) = notes_table;
    ...
    if ~ismember(app.Structure_Idx_Name,fieldnames(app.Notes_File))
        % Getting date-time
        t = now;
        d = datetime(t,'ConvertFrom','datenum','Format','dd-MMM-uuuu HH:mm:ss');
...
if ~app.Comparing
    ...
    if ~isempty(app.Notes_edit.Value{1})
        ...
        app.Notes.(app.Structure_Idx_Name).Notes{strcmp(app.Notes.(app.Structure_Idx_Name).ImgLabel,selected)} = app.Notes_edit.Value{1};
    ...
    if ~isempty(app.Img_one_Edit.Value)
        app.Notes.(app.Structure_Idx_Name).Notes{strcmp(app.Notes.(app.Structure_Idx_Name).ImgLabel,selected1)} = app.Img_one_Edit.Value{1};
    else
        app.Notes.(app.Structure_Idx_Name).Notes{strcmp(app.Notes.(app.Structure_Idx_Name).ImgLabel,selected1)} = {''};
    end
    ...
    try
        ...
        if ~isempty(app.Img_two_Edit.Value)
            app.Notes.(app.Structure_Idx_Name).Notes{strcmp(app.Notes.(app.Structure_Idx_Name).ImgLabel,selected2)} = app.Img_two_Edit.Value{1};
        else
            app.Notes.(app.Structure_Idx_Name).Notes{strcmp(app.Notes.(app.Structure_Idx_Name).ImgLabel,selected2)} = {''};
        end
    end
    writetable(app.Notes.(app.Structure_Idx_Name),app.Notes_File.(app.Structure_Idx_Name));
