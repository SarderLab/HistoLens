...
for st = 1:length(structures)
    ...
    for l = 1:length(labels)
        ...
        if ismember(current_label,app.Aligned_Labels.(structure_idx_name).AllLabels)
            label_idx = find(strcmp(current_label,app.Aligned_Labels.(structure_idx_name).AllLabels));
            % Grab labels and sub-classes (removing unlabeled samples)
            aligned_labels = app.Aligned_Labels.(structure_idx_name).(strcat('Label_',num2str(label_idx))).Aligned;
            aligned_labels = aligned_labels(find(~strcmp(aligned_labels.Class,'Unlabeled')),:);
            
            % Getting features for ranking
            feature_table = innerjoin(app.base_Feature_set.(structure_idx_name),aligned_labels);
            feature_table.ImgLabel = [];

            if ~isnumeric(aligned_labels.Class)
                sub_classes = app.Aligned_Labels.(structure_idx_name).(strcat('Label_',num2str(label_idx))).Sub_Class;
                ...
                for sc = 1:length(sub_classes)
                    ...
                    if ~strcmp(current_sub,'Unlabeled')
                        ...
                        if strcmp(app.Feat_Rank.(structure_idx_name).Method,'chi-square')
                            feature_ranks = fscchi2(rankings_features,'Class');
                        elseif strcmp(app.Feat_Rank.(structure_idx_name).Method,'MRMR')
                            feature_ranks = fscmrmr(rankings_features,'Class');
                        elseif strcmp(app.Feat_Rank.(structure_idx_name).Method,'ReliefF')
                        ...
                        sub_label = strcat('SubClass_',num2str(width(app.Feat_Rank.(structure_idx_name).Table)+1));
                        cat_table = table(ordered_ranks,'VariableNames',{sub_label});

                        app.Feat_Rank.(structure_idx_name).Table = [app.Feat_Rank.(structure_idx_name).Table,cat_table];
                        app.Feat_Rank.(structure_idx_name).AllRanks = [app.Feat_Rank.(structure_idx_name).AllRanks;{current_sub}];
                    end
                end
            else
                ...
                app.Feat_Rank.(structure_idx_name).Table = [app.Feat_Rank.(structure_idx_name).Table,cat_table];
                app.Feat_Rank.(structure_idx_name).AllRanks = [app.Feat_Rank.(structure_idx_name).AllRanks;{current_label}];
            end
        end
    end
end
