...
for st = 1:length(structure_list)
    ...
    structure_info.(structure_name).FeatureSet = Special_Struct2Table(exp_struct.Structure.(structure_name).FeatureSet);
    
    % Loading metadata labels from experiment file (duplicating for
    % slide-level labels)
    if ismember('Aligned_Labels',fieldnames(exp_struct.Structure.(structure_name)))
        labels = fieldnames(exp_struct.Structure.(structure_name).Aligned_Labels);
        for f = 1:length(labels)
            label_name = labels{f};
            if ~strcmp(label_name,'AllLabels')
                structure_info.(structure_name).Aligned_Labels.(label_name).Aligned = Special_Struct2Table(exp_struct.Structure.(structure_name).Aligned_Labels.(label_name).Aligned);
                structure_info.(structure_name).Aligned_Labels.(label_name).Sub_Class = exp_struct.Structure.(structure_name).Aligned_Labels.(label_name).Sub_Class;
                structure_info.(structure_name).Aligned_Labels.(label_name).Name = exp_struct.Structure.(structure_name).Aligned_Labels.(label_name).Name;
            end
        end
        structure_info.(structure_name).Aligned_Labels.AllLabels = cellstr(exp_struct.Structure.(structure_name).Aligned_Labels.AllLabels');
    end

    % Loading structure-level label feature rankings (Load method string
    % separately)
    if ismember('Feat_Rank',fieldnames(exp_struct.Structure.(structure_name)))
        structure_info.(structure_name).Feat_Rank.Table = Special_Struct2Table(exp_struct.Structure.(structure_name).Feat_Rank.Table);
        structure_info.(structure_name).Feat_Rank.Method = exp_struct.Structure.(structure_name).Feat_Rank.Method;
        structure_info.(structure_name).Feat_Rank.Scores = exp_struct.Structure.(structure_name).Feat_Rank.Scores;
        structure_info.(structure_name).Feat_Rank.AllRanks = cellstr(exp_struct.Structure.(structure_name).Feat_Rank.AllRanks');
    end
end
